        ...
        return (
            <LayoutBlock>
                <Datepicker {...this.props}
                    ...
                    startDate = {this.isDateValid(this.props.datepickerState.startDate) ? new Date(this.props.datepickerState.startDate) : null}
                    endDate = {this.isDateValid(this.props.datepickerState.endDate) ? new Date(this.props.datepickerState.endDate) : null}
                    onChange = {this.onChange}
                    customInput = {
                        <fieldset className={'DatePickerInput FlexRow FlexSplit'}>
                            { this.props.label !== S.Strings.EMPTY && (
                                <>
                                    <legend className = {'DatePickerFieldLabel'}>{ this.props.label }</legend>
                                </>
                            )}
                            <div className={'DatePickerSmallLetters'}>от</div>
                            <div className={'DatePickerInputText'}> { this.formatDate(this.props.datepickerState.startDate) } </div>
                            <div className={'DatePickerSmallLetters'}>до</div>
                            <div className={'DatePickerInputText'}> { this.formatDate(this.props.datepickerState.endDate) } </div>
                            {this.isDateValid(this.props.datepickerState.startDate)
                                ? <div onClick={this.onClickClearDates} className={'DateClearButton StartRight SVG Clickable'}>
                                    <SvgClose />
                                </div> : ''}
                        </fieldset>
                    }
...
export class RangeDatepickerState {
    ...
    constructor(startDate: number = S.NOT_EXISTS, endDate: number = S.NOT_EXISTS, open: boolean = false) {
