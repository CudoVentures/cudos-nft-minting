...
class DevGeneratedModule {
    ...
    static processPages(targetPathObj) {
        ...
        for (let i = versions.length; i-- > 0;) {
            ...
            FileHelper.traversePath(targetPath, '', null, false, (sourceChildPath) => {
                ...
                const obj = DevGeneratedModule.requireFileWithoutTransform(sourceChildPath);
                const name = obj.name.replace(/([A-Z])/g, '_$1').toUpperCase().substr(1).replace('PAGE_', '').replace('_H', '');
                frontendPages[name] = obj.URL;
            });

            fs.writeFileSync(path.join(Config.Path.Builds.DEV_GENERATED, info.fileName), `const ${info.objName} = ${JSON.stringify(frontendPages)}; export default ${info.objName};`);
        }
    }
    ...
    static processApis(targetPathObj) {
        ...
        FileHelper.traversePath(sourcePath, '', null, false, (sourceChildPath) => {
            ...
            const name = obj.name.replace(/([A-Z])/g, '_$1').toUpperCase().substr(1).replace('_API_H', '');
        ...
        fs.writeFileSync(path.join(Config.Path.Builds.DEV_GENERATED, '/Apis.ts'), `const Apis = ${JSON.stringify(frontendApis)}; export default Apis;`);
        fs.writeFileSync(path.join(Config.Path.Builds.DEV_GENERATED, '/Actions.ts'), `const Actions = ${JSON.stringify(frontendActions)}; export default Actions;`);
    }
    ...
    static processParams(targetPathObj) {
        ...
        fs.writeFileSync(path.join(Config.Path.Builds.DEV_GENERATED, '/Params.ts'), `const Params = ${JSON.stringify(params)}; export default Params;`);
    }
    ...
    static processModules(targetPathObj) {
        FileHelper.traversePath(targetPathObj.Backend.MODULES, Config.Path.Builds.DEV_GENERATED, null, false, (sourceChildPath, targetChildPath) => {
            const model = DevGeneratedModule.requireFileWithoutTransform(sourceChildPath);
            const frontendObj = {};
            ...
            targetChildPath = targetChildPath.replace('.h', '').replace('.ts', '').replace('.js', '').replace('Model', '');
            targetChildPath = `${targetChildPath}Consts.ts`;
    ...
    static processCountries(targetPathObj) {
        let countriesFileContent = fs.readFileSync(path.join(targetPathObj.Backend.UTILITIES, 'Countries.js')).toString();
        countriesFileContent = countriesFileContent.substr(0, countriesFileContent.lastIndexOf('module.exports'));

        fs.writeFileSync(path.join(Config.Path.Builds.DEV_GENERATED, 'Countries.ts'), `${countriesFileContent} export default countries;\nexport { map as CountriesMap };`);
    }
    ...
    static matchAndProcess(targetPath, targetPathObj) {
        ...
        if (targetPath.indexOf(path.join(Config.Path.Root.BACKEND, '/requests/network-request')) === 0) {
            DevGeneratedModule.processNetwork(targetPathObj);
            return true;
        }
        if (targetPath.indexOf(path.join(Config.Path.Root.BACKEND, '/requests/network-response')) === 0) {
            DevGeneratedModule.processNetwork(targetPathObj);
            return true;
        }
